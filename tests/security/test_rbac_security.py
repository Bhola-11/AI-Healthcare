...
class RBACSecurityPolicyTestCase(TestCase):
    ...
    def test_audit_log_hash_chain_tamper_detection(self):
        ...
        log2 = AuditService.log_action(actor=self.admin_user, action=AuditLog.Action.VIEW, resource=self.patient)
        
        status = AuditService.verify_chain_integrity()
        self.assertTrue(status["is_valid"])

        # Tamper with log1
        AuditLog.objects.filter(id=log1.id).update(details_snapshot={"tampered": True})
